...
void Timer1A_Handler(){
  TIMER1_ICR_R = TIMER_ICR_TATOCINT;// acknowledge TIMER1A timeout
	...
	if ( temp <= t) {
		Sound_Play(song, temp) ;
	}
	temp++ ;
	/*
	int frequency = song[k] ; 
	int period = 80000000/(frequency * 32) ;
	//TIMER1_TAILR_R = period;               // execute user task
	DAC_Out(sinArray[i]); // argument is array element
		if (i < 31) {
			i++ ;
		} else {
			i = 0 ;
